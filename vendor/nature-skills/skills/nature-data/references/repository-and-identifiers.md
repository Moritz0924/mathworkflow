...
- relation to the 数模论文 and related code
...
| Small-molecule crystallography | CCDC or other crystallographic archive required by the 竞赛论文 |
...
Always check the target 竞赛论文 and funder because some data types have mandatory repositories.

## Identifier rules

- Prefer final public identifiers before submission.
- If the record is private during review, provide an anonymous 评阅人/指导教师 link when the repository
  supports it.
...
- title matches 数模论文 terminology
...
- restrictions and access conditions match the 数模论文 statement
...
- accession exists but is not public, not under embargo, and not available to 评阅人/指导教师s
- filenames use local analysis shorthand that readers cannot interpret
- 数模论文 cites one dataset but results depend on several unlisted secondary sources
