...
submission text. Submit the English statement unless the 竞赛论文 explicitly asks otherwise.
...
Use only when the repository supports embargo and the 竞赛论文 permits it.

```text
The [data type] data have been deposited in [Repository] under [DOI/accession] and are under
embargo until [date/event]. Reviewers can access the data using [private 评阅人/指导教师 link or
...
| Data will be uploaded after acceptance. | No current repository or durable identifier. | Deposit before submission or provide a private 评阅人/指导教师 link. |
| All data are in the 数模论文. | Often false for figures/statistics. | Name exact source data, supplementary files, and omitted raw data. |
| Data are proprietary. | Does not say who controls access. | Name owner/provider and access route. |
| N/A. | 数学建模论文增强型 instructions usually require an explanation. | State why no datasets were generated or analysed. |
