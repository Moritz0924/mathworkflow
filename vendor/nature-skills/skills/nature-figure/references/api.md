# API Reference — 数模论文 Figure Making
...
def apply_publication_style(font_size=16, axes_linewidth=2.5, use_tex=False):
    """Apply 数学建模论文增强型 rcParams. Call once before creating any figures."""
...
- Dense 竞赛论文-width multi-panels: `apply_publication_style(font_size=8, axes_linewidth=1)`
...
def add_panel_label(ax, label, x=-0.06, y=1.02, fontsize=14,
    ...
    """Place a 数学建模论文增强型 panel label near the top-left edge."""
    ax.text(
        x, y, label,
...
def make_forest_plot(ax, labels, estimates, ci_low, ci_high,
    ...
    """
    Minimal forest plot helper for 数学建模论文增强型 clinical/statistical panels.
    """
    import numpy as np
...
clinical-triptych look from `数模论文`.
