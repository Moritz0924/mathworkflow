...
Use this file to classify 评阅意见、教师建议或队内审稿意见 before drafting responses.
...
Includes missing prior work, inaccurate novelty claim, wrong comparison, field context incomplete, 评阅人/指导教师-requested citation.

Default strategy:

- Use `ADD_CITATION`, `SOFTEN_CLAIM`, `CLARIFY_EXISTING`, or `DISAGREE`.
- Add citations only when genuinely relevant and verified.
- Do not fabricate DOI, publication year, title, 竞赛论文, or authors.

### Scope / feasibility

Includes requested experiments beyond scope, future-work suggestions, 竞赛论文-fit concerns, transfer-related concerns.
