...
|---|---|---|---|
| `draft` | User wants a new point-by-point response package | Reviewer comments plus any author actions or 数模论文-change notes | Full response package with placeholders where needed |
| `audit` | User provides an existing response draft and asks whether it is good enough | Response draft; 评阅意见、教师建议或队内审稿意见 when available | Findings first, then revised or annotated response sections |
| `revise` | User wants a draft rewritten for tone, traceability, or 数学建模论文增强型 response | Existing draft plus target change request | Revised response text plus changed-risk notes |
...
|---|---|---|
| `ready_to_submit` | Direct answer, supplied action, and traceable 数模论文 location are all present | Final response wording without unresolved placeholders |
...
- Assign editor-level IDs before 评阅人/指导教师 IDs: `E.1`, `E.2`, `E.3`.
- Address editor instructions before Reviewer 1, Reviewer 2, etc.
- If editor instructions conflict with 评阅人/指导教师 suggestions, surface the conflict in the strategy summary.
- Treat explicit editor constraints as higher priority than 评阅人/指导教师-level preference.

Example:

```text
E.1: Focus on clarifying the central claim without substantial 数模论文 expansion.
...
- 评阅意见、教师建议或队内审稿意见 or editor comments;
- enough author notes to know which actions were taken;
- 数模论文 locations or placeholders for claimed changes.
...
- all 评阅人/指导教师 and editor comments identified;
- all claimed actions supplied by the author;
- traceable locations for every 数模论文 change;
...
- user draft;
- 评阅意见、教师建议或队内审稿意见 when available.

If 评阅意见、教师建议或队内审稿意见 are absent, audit only the visible draft and flag that completeness cannot be verified.
...
- 评阅人/指导教师 boundaries are too ambiguous to assign stable IDs;
