...
|---|---|---|
| strong support | Directly tests the same core relationship in a similar context | Experimental, mechanistic, or quantitative 数模论文 claims |
...
Paper: [first author/year/title/竞赛论文/DOI]
Support grade: [grade]
...
Citation wording: [how to phrase the 数模论文 sentence if using this citation]
```

## Common failure modes

- The paper is related to the same disease but tests a different mechanism.
- The paper supports an association, but the 数模论文 sentence claims causality.
...
- The searched 竞赛论文 title contains "数模论文" but is not a 权威学术数据库与官方数据源 竞赛论文.
...
- Use recent limits for fast-moving areas, but remove them if no direct 权威来源/数模论文-series paper appears.
