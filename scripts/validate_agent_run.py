...
def refresh_resolved_validation_queue(run_dir: Path, issues: List[Dict[str, str]]) -> List[Dict[str, str]]:
    queue_path = run_dir / "reports" / "agent_revision_queue.csv"
    rows = read_csv_dict(queue_path)
    current_fail_items = {
        str(issue.get("item") or "")
        for issue in issues
        if str(issue.get("level") or "") == "fail" and str(issue.get("item") or "") != "open_blocking_revision_queue"
    }
    changed = False
    for row in rows:
        status = str(row.get("status") or "").strip().lower()
        ...
        if status == "open" and severity in {"fail", "major"} and match and match.group(1) not in current_fail_items:
            row["status"] = "resolved"
            row["human_decision"] = row.get("human_decision") or "auto_resolved_by_validation"
            notes = str(row.get("notes") or "")
            if "resolved_by_current_validation" not in notes:
                row["notes"] = notes + (";" if notes else "") + "resolved_by_current_validation"
            changed = True
    if changed:
...
def validate_training_outputs(run_dir: Path, issues: List[Dict[str, str]]) -> None:
    ...
    queue_path = run_dir / "reports" / "agent_revision_queue.csv"
    blockers = blocking_queue_rows(refresh_resolved_validation_queue(run_dir, issues))
    for row in blockers:
        add(
            issues,
            "fail",
