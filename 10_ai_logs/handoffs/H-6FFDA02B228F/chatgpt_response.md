...
| 修订任务 | 完成状态 | 本轮修改 | 对应合同或证据 |
|---|---|---|---|
| REV-PR-001 | 已完成 | 在求解方法中补充文内引文 `[1]`、`[2]`，新增完整参考文献，并新增两条可机器登记的 `citation_contract` 数据行；两条来源均完成书目信息核验和摘要阅读，`metadata_verified=true`、`personally_read=true`、`support_grade=method_support`。 | CIT001、CIT002 |
| REV-PR-002 | 已完成 | 将视线垂距符号统一为 \(\delta_{ikjq}(t)\)，并在距离定义、单点遮挡条件和全代表点指示函数中同步修改，不再使用 \(\delta_{ikjq}(t)\)。 | FML007、FML008 |
| REV-PR-003 | 已完成 | 将多弹区间并集恢复为 \(\mathcal U_j=\bigcup_{i,k:\,y_{ik}=1}\mathcal I_{ikj}\)，并在符号表中定义 \(y_{ik}\)。 | FML010 |
| REV-PR-004 | 已完成 | 删除“近优容差”口径，改为先严格求一级目标最优值，再在一级最优解集合上最大化二级目标。 | FML013 |

本轮自检结果：

- 未检出旧视线距离符号 `\delta_{ikjq}`；
- 多弹区间并集显式包含 `y_{ik}=1`；
- 问题五正文不再包含“近优容差”或等价放宽表述；
- 正文存在与参考文献编号一致的 `[1]`、`[2]`；
- `citation_contract` 至少包含一条满足 `metadata_verified=true`、`personally_read=true` 且 `support_grade` 不为 `metadata_only` 的数据行。
