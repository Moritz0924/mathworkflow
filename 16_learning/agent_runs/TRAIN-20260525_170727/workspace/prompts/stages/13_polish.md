# Stage Prompt: polish - Fact-preserving Polish

## 1. Stage Identity

```yaml
stage_id: polish
stage_name: Fact-preserving Polish
stage_order: 13
...
Improve wording while preserving all protected atoms and result meanings.
...
```text
may_update: polish_diff_check.csv | read_only: result_contract.csv; figure_contract.csv; formula_contract.csv; citation_contract.csv
```
...
```text
You are executing stage polish in the v3.2-MVP contract-driven mathematical modeling workflow.
...
```
```bash
python scripts/polish_latex_sections.py --section current --mode deep
```
```bash
...
```text
Do the polished sections preserve all facts while improving readability?
```
...
|---|---|
| Protected atom changed | reject polish. New claim -> remove or return to paper_draft. |
