# Stage Prompt: intake - Problem Intake

## 1. Stage Identity

```yaml
stage_id: intake
stage_name: Problem Intake
stage_order: 1
gate_type: soft
...
Normalize problem statement, attachment inventory, contest metadata, and missing-information risks without solving the problem.
...
```text
You are executing stage intake in the v3.2-MVP contract-driven mathematical modeling workflow.
...
```text
Are the problem statement and attachment inventory complete?
```
...
|---|---|
| Missing file | record in missing_information.md. Ambiguous source -> ask human before downstream assumptions. |
