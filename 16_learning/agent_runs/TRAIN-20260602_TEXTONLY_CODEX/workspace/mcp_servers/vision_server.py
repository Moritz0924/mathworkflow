...
def offline_result(path: Path, prompt: str, risk: str = "") -> Dict[str, Any]:
    return {
        ...
        "risks": [risk or "VISION_OFFLINE or missing QWEN_VL_API_KEY / DASHSCOPE_API_KEY / VISION_API_KEY; no visual semantics were inferred."],
        "source": "offline_metadata",
        "provider": "alibaba_bailian_qwen_vl",
        "prompt": prompt,
        "image_path": str(path),
    }


def first_env(keys: Sequence[str], default: str = "") -> str:
    for key in keys:
...
def chat_completions_endpoint(base_url: str) -> str:
    url = (base_url or DEFAULT_QWEN_VL_BASE_URL).strip().rstrip("/")
...
def call_openai_compatible_vision(path: Path, prompt: str, timeout_seconds: int = 120) -> Dict[str, Any]:
    api_key = first_env(["QWEN_VL_API_KEY", "DASHSCOPE_API_KEY", "VISION_API_KEY"])
    base_url = first_env(["QWEN_VL_BASE_URL", "VISION_BASE_URL"], DEFAULT_QWEN_VL_BASE_URL)
    endpoint = first_env(["QWEN_VL_CHAT_COMPLETIONS_URL", "VISION_CHAT_COMPLETIONS_URL"])
    model = first_env(["QWEN_VL_MODEL", "VISION_MODEL"], DEFAULT_QWEN_VL_MODEL)
    if os.environ.get("VISION_OFFLINE", "").strip().lower() in {"1", "true", "yes"}:
        return offline_result(path, prompt)
    if not api_key:
        return offline_result(path, prompt, "Missing QWEN_VL_API_KEY, DASHSCOPE_API_KEY, or VISION_API_KEY.")
    if not model:
        return offline_result(path, prompt, "Vision model env is incomplete.")

    endpoint = endpoint.strip() or chat_completions_endpoint(base_url)
    instruction = (
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            parsed.setdefault("provider", "alibaba_bailian_qwen_vl")
...
def tool_schema() -> Dict[str, Any]:
    return {
        ...
        "description": "Analyze one image inside the workspace with Alibaba Bailian Qwen-VL and return structured observation JSON.",
        "inputSchema": {
            "type": "object",
            "properties": {
