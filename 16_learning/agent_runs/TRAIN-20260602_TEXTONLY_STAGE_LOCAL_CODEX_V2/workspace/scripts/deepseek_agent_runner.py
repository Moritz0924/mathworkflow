...
def run_live(args: argparse.Namespace) -> int:
    ...
    for attempt in range(1, retry_count + 2):
        ...
        payload = build_chat_payload(policy, attempt_route, messages)
        request_manifest = {
            "provider": (policy.get("provider") or {}).get("name", "deepseek"),
            "model": attempt_route["model"],
            "profile_id": attempt_route["profile_id"],
            "depth_escalation_reason": attempt_route.get("depth_escalation_reason"),
            "stage": stage,
        ...
        dump_json(call_dir / f"attempt_{attempt:02d}_route.json", attempt_route)
