...
def normalize_simulated_gate_csv(content: str) -> str:
    if "formal_effect" not in (content.splitlines()[0] if content.splitlines() else ""):
        return str(content or "").replace("simulated_none", "none")
    try:
        source = io.StringIO(content)
        rows = list(csv.DictReader(source))
        if not source.getvalue().strip() or not rows:
            return str(content or "").replace("simulated_none", "none")
        fieldnames = list(rows[0].keys())
        if "formal_effect" not in fieldnames:
            return str(content or "").replace("simulated_none", "none")
        for row in rows:
            value = str(row.get("formal_effect") or "").strip().lower()
            if value in {"simulated_none", "none", "no", "simulated-no", "simulated_no"}:
...
def normalize_file_action_content(rel_path: str, content: str) -> str:
    normalized = str(rel_path or "").replace("\\", "/").lower()
    if normalized.endswith("11_review/simulated_human_gate_log.csv"):
        return normalize_simulated_gate_csv(content)
    return content
...
def apply_file_actions(
    ...
    for action in actions:
        ...
        if allowed:
            ...
            content = normalize_file_action_content(rel_path, str(action.get("content") or ""))
