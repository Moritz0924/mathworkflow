...
ALLOWED_STATUSES = {"", "draft", "ready", "frozen", "blocked", "closed", "resolved", "waived", "pass", "validated", "verified"}
ACTIVE_STATUSES = {"ready", "frozen", "pass", "validated", "verified"}
...
def has_claim_result_binding(row: Mapping[str, Any], claim_bindings: Mapping[str, Set[str]]) -> bool:
...
def validate_figure_contract(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        bound_by_claim = has_claim_result_binding(row, claim_bindings or {})
        if not result_id and not has_text(row.get("evidence_source")) and not bound_by_claim:
...
def validate_contract_bus(stage: str = "current", all_requirements: bool = False) -> List[Dict[str, str]]:
    ...
    validate_figure_contract(figure_rows, result_ids, issues, claim_bindings)
