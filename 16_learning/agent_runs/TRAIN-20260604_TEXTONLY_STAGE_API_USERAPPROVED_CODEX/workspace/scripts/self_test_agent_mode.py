...
from deepseek_agent_runner import apply_file_actions, extract_strict_json_object, protocol_issues_for_response, sanitized_call_id  # noqa: E402
from run_agent_mode import QUEUE_FIELDS, merge_contract_failures_into_queue, refresh_training_final_submit, stages_for_blockers  # noqa: E402
from validate_agent_run import figure_output_paths, normalize_figure_id, paper_figure_references, topic_alignment_issue  # noqa: E402
...
def test_contract_aliases_and_indirect_figure_binding() -> None:
    with tempfile.TemporaryDirectory() as tmp_raw:
        ...
        with (contracts / "claim_evidence_map.csv").open("w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["claim_id", "claim_text", "evidence_type", "evidence_ref", "status"])
            writer.writeheader()
            writer.writerow(
                {
                    "claim_id": "C001",
        ...
        with (contracts / "figure_contract.csv").open("w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["figure_id", "file_path", "caption", "claim_ids"])
            writer.writeheader()
...
def test_gate_file_action_normalizes_formal_effect() -> None:
    with tempfile.TemporaryDirectory() as tmp_raw:
        ...
        content = "timestamp,stage,gate_type,decision,comment,formal_effect\n2026-06-03T00:00:00Z,intake,human_gate,approved,simulated,simulated_none\n"
