...
VALIDATION_STAGE_MAP = {
    ...
    "training_final_paper_too_thin": ["paper_full"],
    "training_final_paper_placeholder": ["paper_full"],
    "training_final_paper_missing_section_signal": ["paper_full"],
    "training_contract_empty": ["results_freeze", "paper_full"],
    "training_figure_contract_missing_output": ["figures"],
    "training_figure_contract_file_missing": ["figures"],
    "training_claim_missing_result_link": ["results_freeze", "paper_full"],
    "training_claim_missing_figure_link": ["figures", "paper_full"],
    "training_claim_missing_formula_link": ["paper_full"],
    "training_paper_unregistered_figure_reference": ["figures", "paper_full"],
...
def merge_validation_failures_into_queue(run_dir: Path, validation: Mapping[str, Any]) -> int:
    ...
    for issue in validation.get("issues") or []:
        ...
        rows.append(
            {
                "task_id": f"{run_dir.name}-VAL-{added:03d}",
                "run_id": run_dir.name,
...
def build_training_feedback_bundle(policy: Mapping[str, Any], requested_run_id: str, formal_stage: str) -> Dict[str, Any]:
    ...
    if enhancement_rows:
        enhancement_rows.sort(
            key=lambda row: (
                severity_order.get(str(row.get("severity") or "").lower(), 99),
                str(row.get("target_area") or ""),
                str(row.get("enhancement_id") or ""),
            )
        )
            ...
            feedback_rows.append(
                {
                    ...
                    "source_task_id": row.get("enhancement_id", ""),
                    "source_run_id": run_id,
                    "formal_stage": formal_stage,
                    "severity": row.get("severity", ""),
                    "formal_target": feedback_target_from_enhancement(row),
                    "sandbox_target": evidence,
                    "issue_summary": f"validated training enhancement ({target_area or 'workflow'}): {row.get('proposed_change', '')}",
                    "proposed_action": row.get("proposed_change", ""),
...
def run_training_sandbox(args: argparse.Namespace, policy: Mapping[str, Any]) -> int:
    ...
    if validation_queue_added:
        update_status(run_dir, "validation_queue_seeded", {"initial_validation_queue_added": validation_queue_added})
