...
ALLOWED_STATUSES = {"", "draft", "ready", "frozen", "blocked", "closed", "waived"}
ACTIVE_STATUSES = {"ready", "frozen"}
CLOSED_STATUSES = {"closed", "waived"}
...
def validate_figure_contract(rows: Sequence[Mapping[str, Any]], result_ids: Set[str], issues: List[Dict[str, str]]) -> None:
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        outputs = [row.get("output_svg"), row.get("output_png"), row.get("output_pdf")]
        if not any(path_exists(x) for x in outputs if has_text(x)):
            add(issues, "fail", "figure_file_missing", f"{figure_id} has no existing output_svg/output_png/output_pdf", "14_contracts/figure_contract.csv")
