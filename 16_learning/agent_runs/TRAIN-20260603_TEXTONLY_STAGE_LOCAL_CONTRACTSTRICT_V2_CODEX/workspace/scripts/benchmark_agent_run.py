...
def count_existing_registered_figures(workspace: Path) -> int:
    count = 0
    for row in read_csv_rows(workspace / "14_contracts" / "figure_contract.csv"):
        outputs = [str(row.get(field) or "").strip() for field in ("output_svg", "output_png", "output_pdf", "file_path")]
        if any(output and (workspace / output).exists() for output in outputs):
            count += 1
    return count
...
def benchmark_run(run_id: str) -> Dict[str, Any]:
    ...
    our["figure_mentions"] = max(our.get("figure_mentions", 0), count_existing_registered_figures(workspace))
