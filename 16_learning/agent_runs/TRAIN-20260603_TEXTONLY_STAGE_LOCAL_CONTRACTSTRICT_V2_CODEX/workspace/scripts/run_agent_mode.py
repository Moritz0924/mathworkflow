...
VALIDATION_STAGE_MAP = {
    ...
    "training_review_score_below_threshold": ["auto_review", "final_export"],
...
def validation_issue_action(item: str, detail: str) -> Tuple[str, str]:
    actions = {
        ...
        "training_review_score_below_threshold": (
            "Update review scoring only after actual sandbox fixes raise the artifact quality above threshold.",
            "Every review_scorecard row is at least 85% of max_score.",
        ),
...
def run_training_sandbox(args: argparse.Namespace, policy: Mapping[str, Any]) -> int:
    ...
    if args.dry_run:
        ...
        update_status(
            run_dir,
            status,
            {"agent_command_present": True, "initial_exit_code": rc, "initial_log_tail": tail, "initial_synced_reports": initial_synced_reports, **initial_extra},
        )
        if rc != 0:
