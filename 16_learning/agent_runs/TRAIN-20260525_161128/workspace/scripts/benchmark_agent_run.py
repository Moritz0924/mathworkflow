...
from agent_mode_utils import AGENT_RUNS_ROOT, read_agent_policy, safe_rel, write_csv_rows
...
def find_or_build_draft(workspace: Path) -> Tuple[Path, str]:
    ...
    if chunks:
        out = workspace / "09_paper" / "full_draft.md"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(
            "# Sandbox full draft assembled from LaTeX sections\n\n" + "\n\n".join(chunks) + "\n",
            encoding="utf-8",
        )
        return out, "assembled_from_sections"
    out = workspace / "09_paper" / "full_draft.md"
    return out, "missing"
...
def text_features(text: str) -> Dict[str, int]:
    return {
        ...
        "figure_mentions": len(re.findall(r"figure|fig\.|\\includegraphics|图\s*\d+|Fig", text, flags=re.I)),
        "table_mentions": len(re.findall(r"table|表\s*\d+|\\begin\{tabular\}", text, flags=re.I)),
        "formula_mentions": len(re.findall(r"\\begin\{equation\}|\\\[|公式|equation", text, flags=re.I)),
        "validation_mentions": len(re.findall(r"validation|sensitivity|robust|误差|残差|灵敏|检验|验证|稳健", text, flags=re.I)),
