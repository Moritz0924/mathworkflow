# Stage Prompt: task_analysis - Task Decomposition
...
Decompose the problem into subquestions, deliverables, assumptions, dependencies, and modeling requirements.
...
- 04_eda/eda_report.md or no-data note
```
...
```text
You are executing stage task_analysis in the v3.2-MVP contract-driven mathematical modeling workflow.
...
```bash
python scripts/decompose_problem.py
```
```bash
...
```text
Is the decomposition aligned with the official problem deliverables?
```
...
|---|---|
| Ambiguous subquestion | record alternatives and ask human. Missing mapping -> block related downstream claims. |
