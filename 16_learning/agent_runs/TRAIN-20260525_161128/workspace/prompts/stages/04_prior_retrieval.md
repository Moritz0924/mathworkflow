# Stage Prompt: prior_retrieval - Prior Card Retrieval

## 1. Stage Identity

```yaml
stage_id: prior_retrieval
stage_name: Prior Card Retrieval
stage_order: 4
gate_type: soft
...
Extract reusable modeling experience as structured prior cards without copying prior paper text.
...
```text
may_update: none at P0 | read_only: 14_contracts/*.csv
```
...
```text
You are executing stage prior_retrieval in the v3.2-MVP contract-driven mathematical modeling workflow.
...
```text
Are the prior cards useful as hints without copying wording or structure?
```
...
|---|---|
| Copy risk | rewrite as abstract card fields or discard. No match -> record no-match and continue. |
