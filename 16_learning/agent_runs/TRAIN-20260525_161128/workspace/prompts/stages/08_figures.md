# Stage Prompt: figures - Result-bound Figures

## 1. Stage Identity

```yaml
stage_id: figures
stage_name: Result-bound Figures
stage_order: 8
...
Design and generate figures only when they support registered results, evidence, or claims.
...
```text
may_update: figure_contract.csv | read_only: result_contract.csv; claim_evidence_map.csv
```
...
```text
You are executing stage figures in the v3.2-MVP contract-driven mathematical modeling workflow.
...
```text
Which figures should be promoted into the paper, and which should remain exploratory?
```
...
|---|---|
| File missing | regenerate or block row. No evidence binding -> return to results_freeze or keep exploratory. |
