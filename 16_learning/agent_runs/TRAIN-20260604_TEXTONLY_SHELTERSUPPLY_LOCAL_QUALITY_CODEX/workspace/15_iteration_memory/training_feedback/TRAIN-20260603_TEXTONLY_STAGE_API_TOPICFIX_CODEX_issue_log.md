...
This log records an earlier intermediate revalidation. It is no longer the final acceptance record after stricter contract/reviewer checks were added. The later strict API run `TRAIN-20260603_TEXTONLY_STAGE_API_CONTRACTINTEGRATED_CODEX` failed validation, and a subsequent live DeepSeek rerun was blocked by environment policy against external disclosure. The current accepted safe run is `TRAIN-20260603_TEXTONLY_STAGE_LOCAL_FINALFIX_CODEX`; see `15_iteration_memory/training_feedback/TRAIN-20260603_TEXTONLY_STAGE_LOCAL_FINALFIX_CODEX_issue_log.md`.
...
- status_after_revalidation: superseded; not a final acceptance source
