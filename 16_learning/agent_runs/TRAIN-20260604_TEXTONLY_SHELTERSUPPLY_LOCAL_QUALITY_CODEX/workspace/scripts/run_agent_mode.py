...
def find_reused_problem_source(problem: Path | str, runs_root: Path = AGENT_RUNS_ROOT) -> List[Dict[str, str]]:
    problem_path = Path(problem)
    if not problem_path.is_absolute():
        problem_path = ROOT / problem_path
    if not problem_path.exists() or not problem_path.is_file():
        return []
    ...
    if not runs_root.exists():
        return matches
    for lock_path in sorted(runs_root.glob("*/reports/problem_source_lock.json")):
        try:
            lock = json.loads(lock_path.read_text(encoding="utf-8", errors="ignore"))
        except Exception:
            continue
        source = lock.get("source") or {}
        if str(source.get("sha256") or "") != sha:
            continue
        run_id = lock_path.parents[1].name
        matches.append(
            {
                "run_id": run_id,
                "source_path": str(lock.get("source_path") or ""),
                "title": str(source.get("title") or ""),
                "lock_path": safe_rel(lock_path),
            }
        )
    return matches
...
def run_training_sandbox(args: argparse.Namespace, policy: Mapping[str, Any]) -> int:
    ...
    reused_problem_sources = find_reused_problem_source(Path(args.problem)) if args.problem else []
    if reused_problem_sources and not bool(getattr(args, "allow_reused_problem", False)):
        payload = {
            "run_id": run_id,
            "status": "reused_problem_rejected",
            "problem": args.problem,
            "matches": reused_problem_sources[:10],
            "reason": "training sandbox requires a problem statement SHA that has not appeared in prior runs; pass --allow-reused-problem to override",
        }
    ...
    write_training_acceptance_reports(run_dir, training_acceptance)
    if training_acceptance.get("fail_count"):
        final_status = "completed_with_acceptance_issues" if final_status == "completed" else final_status
...
def main() -> None:
    ...
    parser.add_argument("--allow-reused-problem", action="store_true", help="Allow training with a problem statement SHA that appeared in a previous run.")
