...
def validate_latex(issues: List[Dict[str, str]]) -> None:
    ...
    for path in sorted(tex_dir.glob("*.tex")):
        ...
        if re.search(r"TODO|待补|占位|禁止直接提交", text):
...
def validate_legacy_figure_registry(issues: List[Dict[str, str]]) -> None:
    ...
    if not reg.exists():
        add(issues, "warn", "figure_registry", "08_figures/figure_registry.csv is missing; v3.2 uses figure_contract.csv as source of truth", "08_figures/figure_registry.csv")
        return
